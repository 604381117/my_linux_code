#include"ftp_sever_func.h"

void Clientconnectblog(struct sockaddr_in *client)
{
	char buf[1000];
	...
	sprintf(buf,"%04d-%02d-%02d %02d:%02d:%02d client ip:%s port:%d\n",\
		(1900+pTm->tm_year),(1+pTm->tm_mon),pTm->tm_mday,(8+pTm->tm_hour),\
		pTm->tm_min,pTm->tm_sec,inet_ntoa(client->sin_addr),ntohs(client->sin_port));
	if(-1==(fd=open(CLIENTBLOG,O_WRONLY|O_CREAT|O_APPEND,0660)))
	{
		perror("open");
		return;
	}
...
void Clientmanualblog(char *blog)
{
	char buf[1000];
	bzero(buf,sizeof(buf));
	int fd;
	...
	sprintf(buf,"%04d-%02d-%02d %02d:%02d:%02d %s",\
		...
		(8+pTm->tm_hour),pTm->tm_min,pTm->tm_sec,blog);
	if(-1==(fd=open(CLIENTBLOG,O_WRONLY|O_CREAT|O_APPEND,0660)))
	{
		perror("open");
		return;
...
void Loginconfirm(ptda t,int new_sfd,char p[])
{
	bzero(t,sizeof(tda));
	struct spwd *sp;
	char salt[512]={0};
	char buf1[50]={0},buf2[50]={0};//拼接用户名和密码
	bzero(buf1,sizeof(buf1));
	bzero(buf2,sizeof(buf2));
	Recvn(new_sfd,(char*)&t->len,4);
	Recvn(new_sfd,t->buf,t->len);
	if(NULL==(sp=getspnam(t->buf)))
	{
		t->len=-331;//发一个结束标示符
		...
		strcat(buf1,t->buf);//username
		t->len=331;
		Sendn(new_sfd,(char*)&t->len,4);
		Recvn(new_sfd,(char*)&t->len,4);
		Recvn(new_sfd,t->buf,t->len);
		get_salt(salt,sp->sp_pwdp);
		if(strcmp(sp->sp_pwdp,crypt(t->buf,salt))== 0)
		{
			strcpy(buf2," userpasswd:");//userpasswd
			strcat(buf2,t->buf);
			strcat(buf1,buf2);
			memcpy(p,buf1,sizeof(buf1));
			t->len=230;
			Sendn(new_sfd,(char*)&t->len,4);
			printf("验证通过!\n");
		}else{
